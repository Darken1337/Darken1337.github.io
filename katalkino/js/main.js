...
$(document).on('click', function(e){
    ...
    console.log($(e.target).parents('#cart-body').length);

    if($(e.target).attr('id') !== 'open-cart' &&
        $(e.target).parents('#open-cart').length === 0 &&
        $(e.target).attr('id') !== 'cart-body' &&
        $(e.target).parents('#cart-body').length === 0)
        {
            $('#cart-body').hide();
        }
